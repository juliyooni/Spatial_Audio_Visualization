...
`split_channels(data)`. (channels, samples)을 받아 4개를 만든다.

- **Left** = `data[0]`
- **Right** = `data[1]` (모노라면 left 복제)
- **Mid** = `(L + R) / 2` — 가운데에 정위된 성분 (보컬 / 베이스)
- **Side** = `(L - R) / 2` — 좌우 차이만 남은 성분 (스테레오 와이드 신스 / 리버브 꼬리)

각 채널은 모노이지만 stereo 플레이어와 호환되도록 자기 자신을 두 번 쌓아 (2, samples) 형태로 반환. 즉시 계산 — Demucs와 달리 모델 로딩이 없다.
